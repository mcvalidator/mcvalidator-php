...
class NullableShapeOf
{
    ...
    private static function validate(Dict $options)
    {
        foreach ($options as $key => $value) {
            if (!is_string($key)) {
                throw new \Exception("`nullable_shape_of` `\$options` must be a key-value array with string keys");
            }
    ...
    public static function build(Dict $options, ...$sections): Builder
    {
        ...
        return new NullableShapeOfBuilder($options, $sections);
    }
}